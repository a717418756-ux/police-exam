/* ══════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────
   近期版本異動：
     v83  新增類股廣度區塊（37類股指數漲跌計數，資料來自worker.js/
          Code.gs的fetchSectorBreadth，本檔只負責渲染m.tw.breadth）
          含假強/假弱偵測（加權漲跌 vs 類股廣度背離）
   ⚠️ 已知地雷／注意事項：
     - 假強/假弱偵測讀取m.tw.index.changePct（不是chgPct！v83上線時
       曾因欄位名寫錯導致偵測永遠沉默不觸發，不報錯只是不顯示——這類
       「靜默失能」的雷最危險，新增依賴後端欄位的邏輯務必寫最小驗證）
