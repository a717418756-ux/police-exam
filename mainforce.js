...
function computeIntentAnalysis(D, formulas, mainForce) {
  ...
  const scriptMap = {
    '洗盤': {
      title: '🌊 疑似洗盤（洗散戶，非真跌）',
      script: '主力用殺低嚇出散戶籌碼、籌碼未明顯離開——這是量價結構描述（Wyckoff Test等），非方向預測。19年24檔驗證（5,111事件）：方向命中51%≈隨機（法人籌碼同向與否亦無分化），但逐股差異大——本系統已改逐股α閘控，此股歷史有實證優勢才會在行為鏈投方向票（下方會標註）。操作：不宜貿然追空（結構上有承接痕跡），做多進場等吸籌確認觸發價，嚴設停損。',
    },
    '出貨': {
      title: '📉 疑似出貨（真跌，籌碼在離開）',
      script: '主力邊拉邊倒或帶量出逃的量價結構描述。19年24檔驗證（1,509事件）：後5日僅47%真跌（反指標傾向；法人同步賣超時反而更易反彈46%）——絕不可單獨憑此做空，紀律門已對此除權。操作：出貨結構下做多應提高警覺、減碼觀察；做空需靠其他證據（籌碼、環境、溫度計初期）共振，此判定僅代表量價結構弱。',
    },
    '進貨': {
      title: '📈 疑似進貨（低檔布局）',
