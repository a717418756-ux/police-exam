/* ══════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────
   近期版本異動（供排雷定位用）：
     v76  FUSION極端值標籤大改：≥40從「多方共振偏漲/buy」改為「動能極強
          結構—非買進訊號/hold」；≤-40移除反彈獲利暗示。原因：19年
          75,056→116,759樣本驗證，≥40後5日α-2.5（反而略偏回檔）、
          ≤-40後5日α+1.7（反而略偏反彈）——兩端皆非方向訊號
     v88  數值更新至24檔19年最終版（116,759樣本，見help.js/reader.md）
   ⚠️ 已知地雷／注意事項：
     - calcFusion的signal欄位('buy'/'sell'/'hold')僅影響app.js顯示顏色，
       不代表方向優勢已驗證——極端值(|fusion|≥40)已證實無方向優勢，
       signal='hold'是刻意設計，勿改回'buy'/'sell'除非有新的大樣本證據
     - FUSION權重(STI/MFD/ECO係數)不可依單一批次股票調整，本專案已有
       4次「小樣本看似有效、大樣本證偽」教訓，任何權重異動須19年24檔
       複驗（用backtest_standalone.js的FUSION驗證區塊）
   ══════════════════════════════════════════════════════════════════════ */
/* v163 檔案版本宣告：讓前端能查出「站上哪個檔案沒更新到」。
   改這個檔時一併把數字改成當版；config.js 的 FILE_VERS 必須同步（自我檢查會擋）。 */
try { (window.SR_FV = window.SR_FV || {})['formula.js'] = 163; } catch (e) {}
