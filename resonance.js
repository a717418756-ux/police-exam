/* ══════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────
   函式清單：
     computeAutocorrelation — 個股性格檢定（Lag-1自相關+Bartlett顯著性）
                              ＝Trend Persistence（動能延續/反轉型/隨機游走）
     computeResonance        — 六維度共識度計算（regime動態調權重）
     renderResonance         — 共振卡渲染
   ⚠️ 已知地雷／注意事項：
     - computeAutocorrelation回傳的character（動能延續/反轉/隨機）會被
       bingfa.js行為推理鏈引用為「個股性格」證據，dir=0不投方向票
       （純統計描述不代表方向），修改此函式輸出格式須同步檢查bingfa.js
     - computeResonance的權重依regime(市場狀態)動態調整，勿寫死固定
       權重，否則喪失「趨勢盤看動能、盤整盤看反轉」的核心設計
   ══════════════════════════════════════════════════════════════════════ */
