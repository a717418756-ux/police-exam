...
try { (window.SR_FV = window.SR_FV || {})['quant.js'] = 163; } catch (e) {}
...
// ──────────────────────────────────────────────────────────────────────
// 函式清單：
//   signalsAtIndex     — 給定索引i，算該日所有訊號快照（給回測用）
//   backtestWeights     — 逐股回測動態權重（此股歷史每指標命中率）
//   computeProprietaryScore — 大漲/大跌專屬分數（加權合計）
//   contrarianSignal    — 韭菜反指標（情緒極端時反向）
//   _avg/_btSMA/_btEMAseries/_btRSI/_btKD/_btMACD/_btBB/_btWR
//                       — 回測用輕量技術指標（_bt前綴），與app.js正式版
//                         分離以避免回測時互相污染或重複計算
// ──────────────────────────────────────────────────────────────────────
// ⚠️ 已知地雷／注意事項：
//   - 本檔_bt系列函式為quant.js內部依賴鏈起點，backtest_standalone.js等
//     外部腳本若要重用這些函式，須整檔eval quant.js（不能只抽單一函式），
//     因為_btMACD依賴_btEMAseries、其他函式間也有依賴，缺一則
//     ReferenceError（純語法檢查node -c抓不到，屬runtime錯誤）
//   - 本檔動態權重與交易日誌無關，是用「該股股價歷史」回測，勿與
//     journal.js的凱利勝率（來自真實交易紀錄）混淆
// ══════════════════════════════════════════════════════════════════════
