...
const HELP_DB = {
  ...
  mainforce: {
    title: '🕵️ 主力行為推估',
    what: '整合OBV能量潮背離、法人價格背離、假突破/假跌破、影線、量價，推估主力正在：吸籌/洗盤/出貨/誘多/誘空/恐慌殺盤。內含「主力意圖研判」區塊，專門分辨下跌是洗盤還是出貨。',
    how: 'OBV是「偷跑偵測器」：價平但OBV升=主力偷買；價漲但OBV降=主力偷賣。意圖研判分岔：①洗盤(量縮跌+有下影+OBV沒破+法人沒跑+散戶恐慌)=洗散戶將漲，不宜追空；②出貨(爆量跌+OBV破底+法人賣+開高走低)=真跌，反彈是逃命波；③進貨(低檔量增OBV升)=打底布局將漲。',
    note: '主力行為屬機率推估非事實，務必與籌碼健康度、共振交叉驗證。「洗盤vs出貨」是短線做空最關鍵的分岔——同樣在跌，洗盤追空會被軋、出貨才是空點。此研判已接入出手紀律門，洗盤時會擋下做空建議。'
  },
  margin: {
    title: '💳 融資融券 · 散戶心理',
