/* ══════════════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────────
   函式清單（29個，重點列出）：
     addBatchRow/calcBatch          — 分批進場計算器
     addExitRow/calcExit             — 分批出場計算器
     openJournal/closeJournal/switchModalTab — 面板開關與分頁切換
     addTradeFromForm                — 新增交易紀錄（表單→dbAddTrade）
     syncWinRateToMain                — 真實勝率(≥5筆)自動回填主頁凱利欄
                                        （模擬單不計入，見db.js鐵律）
     exportMarkdown                   — 匯出完整分析報告（含v65處分效應
                                        偵測區塊），供貼給AI分析用
   ⚠️ 已知地雷／注意事項：
