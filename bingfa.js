        ...
        <div style="font-size:10px;color:var(--muted);line-height:1.7">
          ${(() => {
            ...
            try {
              ...
              return `${(() => {
                /* v134 參數掃描實證：20組停損×目標組合、153筆樣本（空頭排列+反彈至壓力進場）
                   停損 0.5/1.0/1.5/2.0/2.5×ATR × 目標 1/1.5/2/3×ATR，含0.287%做空成本：
                     停損0.5×+目標1× → 淨 +0.020%（唯一正期望值）
                     停損1.0×+目標1× → -0.369%
                     停損2.0×+目標2× → -1.021%（系統原設定所在區間）
                     停損2.5×+目標2× → -1.315%（最差）
                   規律：停損越寬，期望值單調惡化——寬停損讓單次虧損吃掉多次獲利。
                   ⚠️ 但 +0.020% 幾乎等於零、樣本僅153筆，不足以保證獲利；
                      且0.5×ATR極近，實務上會頻繁被雜訊掃損，需要極嚴格的紀律與低成本。
                   這裡如實呈現取捨，不替使用者決定。 */
                return `<div style="margin:6px 0;padding:7px 9px;background:var(--warn)0d;border:1px dashed var(--warn);border-radius:7px;font-size:10px;line-height:1.6">
                  ⚖️ <b>停損寬窄的實測取捨</b>（20組參數×153筆，含成本）：停損 <b>0.5×ATR＋目標1×ATR</b> 是唯一淨期望值為正的組合（+0.020%）；
                  停損放寬到 1×/2×/2.5×ATR 依序為 -0.369%／-1.021%／-1.315%，<b>越寬越差</b>。
                  下方建議停損為結構位計算值（較寬、不易被掃），若你要追求正期望值需改用更近停損——
                  代價是頻繁掃損，且 +0.020% 幾乎等於零、樣本不足以保證獲利。兩者取捨由你決定。
                </div>`;
              })()}
